...
- **Colima's `docker inspect` under-reports ports.** `NetworkSettings.Ports` comes back `{}`
  ...
  `FACTORIO_SERVER_PORT` explicitly (see `.env`) to skip discovery.
- **UNRESOLVED: the Factorio container crash-loops under colima's bind mounts.** It fails to
  write `/opt/factorio/config/config.ini` and copy `mod-list.json` into `/opt/factorio/mods`
  (Permission denied), then can't even read back `server-settings.json` it just wrote
  ("No such file or directory" from a `file_size()` stat call, despite the file existing on
  the host side of the bind mount). `chmod -R 777` on the host `fle/cluster/config` and
  `fle/cluster/mods` directories didn't fix it — smells like a virtiofs bind-mount
  consistency/caching issue specific to colima, not a plain UID mismatch. Next things to try:
  Docker Desktop instead of colima (the officially-tested path), or `colima start
  --mount-type sshfs` / `--vm-type=vz --vz-rosetta=false` variants, or check if the container
  actually needs `--user root`. First live run is still blocked on this.
